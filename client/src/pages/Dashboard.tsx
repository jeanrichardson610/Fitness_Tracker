...
import { Activity, FlameIcon, HamburgerIcon, Ruler, Scale, ScaleIcon, TrendingUpIcon, ZapIcon } from "lucide-react";
      ...
      <div className="dashboard-grid">
        ...
        <Card className="shadow-lg col-span-2">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="">
              <p className="text-sm text-slate-50 dark:text-slate-400">Calories Consumed</p>
              <p className="">{totalCalories}</p>
            </div>
            <div className="text-right">
              <p className="text-sm text-slate-50 dark:text-slate-400">Limit</p>
              <p className="">{DAILY_CALORIE_LIMIT}</p>
            </div>
          </div>
          ...
          <div className="mt-4 flex justify-between items-center">
            <div className={`px-3 py-1.5 rounded-lg ${remainingCalories >= 0 ? "bg-emerald-50 dark:bg-emerald-900/10 text-emerald-700 dark:text-emerald-400" : "bg-red-50 dark:bg-red-900/10 text-red-700 dark:text-red-400"}`}>
              <span className="text-sm font-medium">
                {remainingCalories >= 0 ? `${remainingCalories} kcal remaining` : `${Math.abs(remainingCalories)} kcal over limit`}
              </span>
            </div>
              <span className="">{Math.round((totalCalories / DAILY_CALORIE_LIMIT) * 100)}%</span>
          </div>
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="">
              <p className="text-sm text-slate-50 dark:text-slate-400">Calories Burned</p>
              <p className="">{totalBurned}</p>
            </div>
            <div className="text-right">
              <p className="text-sm text-slate-50 dark:text-slate-400">Goal</p>
              <p className="">{user?.dailyCalorieBurn || 400}</p>
            </div>
          </div>
          <ProgressBar
            value={totalBurned}
              ...
              {user.height && (
                <div className="pt-2 border-t border-slate-100 dark:border-slate-800">
                  ...
                  <div className="flex justify-between mt-1 text-[10px] text-slate-400">
                    <span className="text-sm font-medium text-slate-700 dark:text-slate-300">18.5</span>
        ...
        <Card>
          ...
          <div className="space-y-3">
            ...
            <div className="flex justify-between items-center py-2 border-b border-slate-100 dark:border-slate-800">
              ...
              <span className="font-medium text-slate-700 dark:text-slate-200">{totalCalories}</span>
            </div>
